...
dev-build-docker: build
	$(DOCKER) build -t emminex23/hotrod-smart-test:latest \
		--platform $(GOOS)/$(GOARCH) \
		.

build-docker: build
	$(DOCKER) build -t emminex23/hotrod-smart-test:$(RELEASE_TAG)-$(GOOS)-$(GOARCH) \
		--platform $(GOOS)/$(GOARCH) \
		--provenance false \
		.

push-docker: build-docker
	$(DOCKER) push emminex23/hotrod-smart-test:$(RELEASE_TAG)-$(GOOS)-$(GOARCH)
...
release-images.txt:
	...
	for os in $(RELEASE_OSES); do \
 		for arch in $(RELEASE_ARCHES); do \
			echo emminex23/hotrod-smart-test:${RELEASE_TAG}-$$os-$$arch >> dist/release-images.txt; \
		done; \
	done;
...
release: build-release release-images.txt tag-release
	...
	$(DOCKER) manifest create --amend emminex23/hotrod-smart-test:$(RELEASE_TAG) \
		$(shell cat dist/release-images.txt)
	$(DOCKER) manifest push emminex23/hotrod-smart-test:$(RELEASE_TAG)
